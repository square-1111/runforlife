...
class DailyDocument:
    ...
    run_is_indoor: bool | None = None       # True = treadmill/indoor (set pace, not GPS-measured)
    run_efficiency_factor: float | None = None  # speed(m/min)/HR — aerobic efficiency (higher = fitter)
    run_avg_cadence: int | None = None       # steps/min — run economy signal (drifts low off the bike)
    ...
    def to_row(self) -> dict:
        ...
        return {
            ...
            "run_avg_cadence":          self.run_avg_cadence,
